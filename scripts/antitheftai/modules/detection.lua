...
function detection.canPlayerSeeNpc(npc, self, nearby, types, config)
    if not self.cell or self.cell.isExterior then return false end
    if not (npc and npc:isValid()) then return false end
    if npc.cell ~= self.cell then return false end
    if types.Actor.isDead(npc) then return false end
    ...
    local playerForward = self.rotation:apply(util.vector3(0, 1, 0))
    local angleToNpc = playerForward:dot(toNpc:normalize())

    if angleToNpc < math.cos(config.LOS_HALF_CONE) then return false end
...
function detection.initializeSneakDetection(I)
    if I and I.SkillProgression and I.SkillProgression.addSkillUsedHandler then
        local success, err = pcall(function()
            I.SkillProgression.addSkillUsedHandler(function(skill, p)
                if skill == 'sneak'
                   and p.useType == I.SkillProgression.SKILL_USE_TYPES.Sneak_AvoidNotice then
                    lastSneakTick = 0
                    log("Sneak successful - hidden")
                end
            end)
        end)
        if success then
            log("Sneak detection handler registered")
        else
            log("WARNING: Failed to register sneak handler:", err)
        end
    else
        log("WARNING: SkillProgression interface not available")
    end
end

function detection.updateSneakTimer(dt, self)
    if not self.controls.sneak then 
        lastSneakTick = math.huge 
    else
        lastSneakTick = lastSneakTick + dt
    end
end
